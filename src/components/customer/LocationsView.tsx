...
import { getFamousCountryImages, getCityImage } from '@/lib/utils/world-data'

const fixBrokenImageUrl = (url: string, type: 'country' | 'city', name: string, parentName?: string) => {
  const isBroken = !url || 
                   url.includes('images.unsplash.com/featured/') || 
                   url.includes('loremflickr.com');

  if (isBroken) {
    if (type === 'country') return getFamousCountryImages(name)[0];
    return getCityImage(name, parentName || '');
  }
  return url;
}

const GLOBAL_FALLBACK = 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=1600&auto=format&fit=crop';

const handleImageError = (e: React.SyntheticEvent<HTMLImageElement, Event>) => {
  const target = e.target as HTMLImageElement;
  if (target.src !== GLOBAL_FALLBACK) {
    target.src = GLOBAL_FALLBACK;
  }
};
...
export default function LocationsView() {
  ...
  const countryBanners = useMemo(() => {
    ...
    // Fix broken URLs
    images = images.map(img => fixBrokenImageUrl(img, 'country', selectedCountry));

    // Fallback if no images provided for this country
    if (images.length === 0) {
        return getFamousCountryImages(selectedCountry);
    }
  ...
  const cityData = useMemo(() => {
    ...
    locations
      ...
      .forEach(l => {
        if (!map.has(l.city)) {
          const fixedImages = (l.cityImages || []).map(img => fixBrokenImageUrl(img, 'city', l.city, l.country));
          map.set(l.city, { images: fixedImages.length > 0 ? fixedImages : [getCityImage(l.city, l.country)], count: 1 })
        } else {
          const current = map.get(l.city)!
          const newFixedImages = (l.cityImages || []).map(img => fixBrokenImageUrl(img, 'city', l.city, l.country));
          const mergedImages = Array.from(new Set([...current.images, ...newFixedImages]))
              ...
              {cityData.map(city => (
                <CarouselItem key={city.name} className="pl-6 basis-full sm:basis-1/2 md:basis-1/3 lg:basis-1/5 overflow-visible">
                  <div className="p-3 overflow-visible">
                    ...
                    >
                      <img
                        src={fixBrokenImageUrl(city.images[0] || '', 'city', city.name, selectedCountry)}
                        onError={handleImageError}
                ...
                <div className="grid grid-cols-1 gap-3 pt-4 border-t border-border">
                  ...
                  <div className="flex items-center gap-3 group/info">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="font-bold text-[9px] text-foreground/80 tracking-wide uppercase leading-none">{loc.hours || '11AM - 2AM'}</span>
                    </div>
                  </div>
                </div>
